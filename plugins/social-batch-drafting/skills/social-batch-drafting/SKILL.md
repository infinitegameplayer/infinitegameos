...
**Step 3. Read prior batches (always-on look-back).** Always scan both your active social drafts directory and your archive directory (wherever completed batches land after the queue runs). Most recent installments typically live in the archive, not the active folder. Build a combined date-sorted view across both and take the top 3 to 5 by date. Two outcomes:

- If `series_predecessor` is present (or a campaign series exists per `campaign` and tags): perform full extraction. Pull which prior batches are already threaded, the current anchor arc state verbatim and the Day 1 opener pattern from the predecessor. Threading Protocol activates in Step 4.
- If no series exists: surface 1 to 2 quotable lines, recurring images or voice anchors from recent batches that could thread subtly into this draft. Present to operator. Operator chooses whether to weave them. Always optional. Never forced.

**Step 4. Threading plan (when series exists).** If predecessor exists, name the batches to thread before drafting. All platforms get the thread, not just one. Draft the Day 1 opener explicitly before any other copy. Skipped cleanly when no series.
...
**Step 7. Draft the 9-post batch.** Three days × three platforms × the canonical structure. Reference the Batch Format section for the table, platform mechanics, headline rules, carousel character budgets, 8-slide carousel architecture and image brief format.
...
- Carousel slide headings: NO terminal punctuation. Zero on any slide including the cover and close. Day 1 and Day 3 single-headline templates carry a period or question mark; carousel headings do not.
- 3 to 6 words preferred. Punchy. Direct. Pulled from the source content, not invented.

### Carousel character budgets

Two slots. Calibrate the specific ceilings to your actual render output; these are the starting defaults.

| Slot | Heading | Body |
|---|---|---|
| Cover (slide 1) | ≤32 chars | ≤170 chars |
| Body (slides 2 to 8, close included) | ≤48 chars | ≤220 chars |

**Halt rule.** If any heading or body exceeds its slot ceiling, halt drafting before passing to Queue. Catching overflows at drafting time is cheaper than re-rendering. The Cover heading ceiling is lower because a longer heading wraps onto a third line and overlaps the body at most template sizes. If your renderer handles it cleanly, calibrate your own ceiling from an actual render test.
...
When Day 3 uses a generated background image, the brief format is: cinematic, specific lighting, landscape and sensory detail, no figures (keeps the central archetype elusive), color palette that bridges your card template's text color, pull cinematic language from a cinematography reference (depth, framing, angle, light) so the background carries story not only atmosphere, photographic realism, 16:9 (1792×1024) for social-friendly framing. Include a one-sentence rationale explaining the visual metaphor and why it carries the batch's concept. The brief lives in the batch draft file. Generation execution belongs to the downstream queue skill.

## Threading Protocol

Default is subtle. Subtle is the only default. The principle: every social post knows what came before it without announcing that it does.

**When a series predecessor exists.** All platforms get the thread, not LinkedIn only. All three. Day 1 opens with what came before and where we are now using temporal language only. Day 2 Carousel Slide 8 closes by threading prior installments. Day 3 across all platforms references prior installments before the anchor seed. No internal session names, no plan names, no protocol references.

**When no series exists.** Step 3 surfaces 1 to 2 candidate breadcrumb lines from recent batches. A quotable phrase echoed in a Carousel slide. A sensory image lightly recalled. A voice anchor carried forward without explicit reference. Always optional. Never forced.

**Too-loud threading patterns (banned).** These break the subtle default.

- Explicit call-outs: "Last week I wrote about X." "Previously on..." "As I mentioned before..."
- Numbered-series framing: "Part 3 of my series on Y." "Week 4 check-in."
- Newsletter-style recaps: a paragraph summarizing prior installments before the current thought.
- Stacking references: "First X, then Y, then Z" that enumerates the prior run in sequence.

Subtle threading lands when the right person feels continuity without being told they're reading a series. Explicit threading reads as a newsletter. The distinction is the register, not the presence of a callback.
...
- **No em dashes.** Replace with a period and a new sentence, or a comma. Absolute. No exceptions anywhere.
...
batch_state: draft  # machine-readable resume pointer; see states below
created: YYYY-MM-DD
...
**`batch_state` values (in order):** `draft` → `voice-cleared` → `approved-pending-queue` → `images-generated` → `desktop-reviewed` → `uploaded` → `scheduled` → `published`. The Queue skill reads `batch_state` at entry and resumes from its corresponding step. Drafting advances state through `approved-pending-queue`. Queue advances from there.
